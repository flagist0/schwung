...
static void init_shadow_shm(void)
{
    ...
    /* Test-bus stream SHMs (Phase 2). Owned by src/host/shadow_test_stream.c
     * so future channels (MIDI_IN, log tail) land there, not here. */
...
static void shim_post_transfer(void *ctx, uint8_t *shadow, const uint8_t *hw, int size)
{
     ...
     * (one atomic load + branch). Implementation in shadow_test_stream.c. */
