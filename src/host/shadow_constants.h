...
#define SHM_TEST_STREAM_MIDI_OUT "/schwung-test-stream-midi-out" /* Shim → schwung-testd MIDI_OUT events */
 ...
 *   - Shim:     schwung_shim.c overtake-exit + shadow_midi.c:shadow_chain_midi_inject
 *   - Test bus: src/host/test_daemon/schwung_testd.c (schwung-testd)
 *
 ...
 *
 * Ring uses monotonic uint32 sequence cursor: writer just keeps
 * incrementing `write_seq` and storing into `buffer[write_seq %
 * CAPACITY]`. Reader tracks its own `last_read` and computes events
 * since then. If the writer ran more than CAPACITY events past the
 * reader, the oldest were overwritten — reader fast-forwards and
 * increments dropped (delta = N missed).
 *
 * Single producer (shim's SPI post-transfer hook) + single consumer
 * (schwung-testd). No CAS needed; ACQUIRE/RELEASE on write_seq pairs
 * with daemon's ACQUIRE load for visibility of buffer writes.
 */
