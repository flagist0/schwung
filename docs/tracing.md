...
- `param.serve` — the shim servicing a `shadow_param` request; emitted as a
  cross-process child of the JS `param.get` span (see [Cross-process
  correlation](#cross-process-correlation)).

**shadow_ui** (`schwung-shadow-ui`), per UI tick:
